			...
			foreach( $files as $file ) {
				if ( substr( $file[0], -1 ) == '/' ) { // Directory.
					...
					*/
					...
					if ( true === $viewable ) {
						echo '<span class="viewlink">View file contents</span>';
					}
		...
		if ( false === $extract_result ) { // failed.
			...
			foreach( $files as $file ) {
				if ( false === rename( $destination . '/' . $file, ABSPATH . $file ) ) {
					$error = 'Error #5658696958. Unable to move restored file `' . $destination . '/' . $file . '` to `' . ABSPATH . $file . '`. Verify permissions on destination location.';
					pb_backupbuddy::status( 'error', $error );
					echo '<script type="text/javascript">jQuery("#pb_backupbuddy_working").hide();</script>';
					die();
				} else {
			...
			pb_backupbuddy::status( 'message', 'File restore complete.' );
			echo '<script type="text/javascript">jQuery("#pb_backupbuddy_working").hide();</script>';
			pb_backupbuddy::flush();
